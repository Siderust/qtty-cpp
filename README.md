...
- ✨ **Type-safe**: Compile-time unit checking prevents dimension errors
- 🎯 **Ergonomic**: Natural operator syntax with user-defined literals
- 🚀 **Zero-overhead**: Header-only template design with inline functions
- 🎨 **Modern C++17**: Leverages constexpr, template metaprogramming, and CTAD
- 🛡️ **Exception-based errors**: Clear exception hierarchy for runtime errors
- 🔧 **Auto-generated**: C++ bindings automatically generated from Rust FFI
...
```
┌─────────────────────────────────────────────┐
│     C++ Template Layer (qtty-cpp)           │
│  ┌────────────────────────────────────────┐ │
│  │ Quantity<UnitTag> template             │ │
│  │ - Type safety, operators, literals     │ │
│  └────────────────────────────────────────┘ │
└─────────────────────────────────────────────┘
                    ↓
┌─────────────────────────────────────────────┐
│    Generated C++ Headers                    │
│  ┌────────────────────────────────────────┐ │
│  │ Tag structs, UnitTraits, type aliases  │ │
│  │ Auto-generated by gen_cpp_units.py     │ │
│  └────────────────────────────────────────┘ │
└─────────────────────────────────────────────┘
                    ↓
┌─────────────────────────────────────────────┐
│      Rust qtty-ffi Library                  │
│  ┌────────────────────────────────────────┐ │
│  │ Unit conversions, C FFI interface      │ │
...
| **Compound** | - | ∞ | Velocity (Length/Time) | Created by division |
...
The build orchestrates three tools: Python → Cargo → CMake.
...
├── gen_cpp_units.py            # C++ wrapper generator
├── README.md                   # This file
├── LICENSE                     # License file
│
├── include/qtty/               # C++ headers (mostly auto-generated)
│   ├── qtty.hpp                # Main entry point
│   ├── ffi_core.hpp            # Quantity<> template
│   ├── literals.hpp            # User-defined literals (generated)
│   └── units/                  # Unit-specific headers (generated)
│       ├── length.hpp          # Length units
│       ├── time.hpp            # Time units
│       ├── angular.hpp         # Angular units
│       ├── mass.hpp            # Mass units
│       ├── power.hpp           # Power units
│       └── velocity.hpp        # Compound velocity units
│
├── examples/
│   └── demo.cpp                # Usage demonstrations
│
├── tests/                      # Google Test suite
│   ├── fixtures.hpp            # Test fixtures
│   ├── main.cpp                # Test entry point
│   ├── test_length.cpp         # Length unit tests
│   ├── test_time.cpp           # Time unit tests
│   ├── test_angular.cpp        # Angular unit tests
│   ├── test_mass.cpp           # Mass unit tests
│   ├── test_power.cpp          # Power unit tests
│   ├── test_derived.cpp        # Compound unit tests
│   ├── test_operations.cpp     # Operator tests
│   ├── test_dimension_safety.cpp  # Exception tests
│   └── test_precision.cpp      # Edge case tests
│
├── docs/                       # Documentation
│   ├── architecture.md         # Architecture deep-dive
│   ├── build-and-testing.md    # Build/test guide
│   ├── extending-units.md      # Adding new units
│   └── api/
│       └── quantities.md       # API reference
│
├── qtty/                       # Git submodule (Rust qtty library)
│   └── qtty-ffi/               # C FFI crate
│       ├── include/
│       │   └── qtty_ffi.h      # C header (generated by Rust)
│       └── target/release/
│           └── libqtty_ffi.*   # Compiled shared library
│
└── build/                      # Build artifacts (created by CMake)
    ├── demo                    # Demo executable
    ├── test_ffi                # Test executable
    └── _deps/                  # Google Test (fetched by CMake)
```
